 ...
 */
void HorizontalKAIN::setupLinearSystem() {
    Timer t_tot;
 ...
 */
void HorizontalKAIN::expandSolution(double prec, OrbitalVector &Phi, OrbitalVector &dPhi, ComplexMatrix *F, ComplexMatrix *dF) {
    Timer t_tot;
    int nHistory = this->orbitals.size() - 1;
    int nOrbitals = this->orbitals[nHistory].size();
